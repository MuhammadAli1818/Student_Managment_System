...
class Student {
  ...
  enroll_course(courseName: string) {
...
class student_manager {
  ...
  add_student(name: string) {
    ...
    console.log(
      `Students: ${name} added successfully. Student ID: ${student.id}`
    );
  }

  // method to enroll a student in a course
  enroll_student(studentId: number, courseName: string) {
  ...
  // method to pay student fees
  pay_fees(studentId: number, amount: number) {
    const student = this.find_student(studentId);
    if (student) {
